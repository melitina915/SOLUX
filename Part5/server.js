...
app.post('/chatroom', checklogin, function(req, res){
    ...
    var savesth = {
        title : '채팅방', 
        member : [ObjectId(req.body.opponentid), req.user._id],
        date : new Date()
    }

    // 콜백함수 대신 .then도 사용 가능
    db.collection('chatroom').insertOne(savesth).then((result) => {
        res.send('성공');
    });
...
app.get('/chat', checklogin, function(req, res){
    // req.user._id는 현재 로그인한 유저의 _id
    db.collection('chatroom').find({ member : req.user._id }).toArray().then((result) => {
    // member : [어쩌구, 저쩌구] 이렇게 저장된 것을 찾는 것도 가능하다
